  ...
  const EventPublishPage = ({ handleInputChange }) => {
    ...
    useEffect(() => {
      ...
      if (refundAllowed) {
        ...
        handleInputChange("refundDate", eventData.createdAt);
              ...
              {refundAllowed && (
                <Flex vertical gap={20}>
                  <Flex vertical gap={15} className={styles.refundDays}>
                    ...
                    <Text className={styles.grayText}>
                      Максимальное значение для вашего мероприятия <span style={{color: "#000"}}>{Math.min(30, dayjs(eventData.endTime).diff(dayjs(), 'day') - 1)} дней</span>
                    </Text>
                    <InputNumber
  placeholder="Дней до окончания события"
  min={1}
  max={Math.max(1, dayjs(eventData.endTime).diff(dayjs(), 'day') - 1)}
  value={refundDays}
  onChange={(value) => {
    const maxDays = dayjs(eventData.endTime).diff(dayjs(), 'day') - 1;
    setRefundDays(Math.min(value, maxDays));
  }}
/>

<Text style={{ color: "red" }}>
  Люди смогут вернуть билеты до {dayjs(eventData.endTime).subtract(refundDays, "day").format("DD.MM.YYYY")}
</Text>

...
const EventPublishPage = ({ handleInputChange }) => {
  const { id } = useParams();
  const { data: eventData, isLoading: eventDataLoading } = useGetEventById(id);

  const refundDate = eventData?.refundDate ? dayjs(eventData.refundDate) : null;
  const [refundAllowed, setRefundAllowed] = useState(!!refundDate);
  const [refundDays, setRefundDays] = useState(refundDate ? dayjs(eventData.startTime).diff(refundDate, 'day') : 3);
  ...
  useEffect(() => {
    if (refundAllowed) {
      const newRefundDate = dayjs(eventData.startTime).subtract(refundDays, "day");
      handleInputChange("refundDate", newRefundDate);
      handleInputChange("isAutoRefund", autoConfirmRefund);
    } else {
      handleInputChange("refundDate", dayjs());
      handleInputChange("isAutoRefund", 0);
    }
  }, [refundAllowed, refundDays, autoConfirmRefund]);
        ...
        <Flex vertical>
            ...
            {refundAllowed && (
              <Flex vertical gap={20}>
                <Flex vertical gap={15} className={styles.refundDays}>
                  ...
                  <InputNumber
                    placeholder="Дней до возврата билета"
                    min={1}
                    max={30}
                    value={refundDays}
                ...
                <Flex vertical>
                  ...
                  />
                  <Text className={styles.grayText}>
                    Если включено, пользователи смогут оформить возврат <br /> без вашего участия, в случае если бюджет вашего <br />  мероприятия позволяет <Text style={{color: "red"}}>(иначе обработка происходит вручную)</Text> 
                  </Text>
                </Flex>
              </Flex>
            )}
          ...
          <Flex className={styles.tariffs}>
            ...
            >
              ...
              <Text className={styles.tariffCard__text}>Публикация мероприятия без дополнительных функций</Text>
              {selectedTariff === 0 && (
            ...
            >
              ...
              <Text className={styles.tariffCard__text}>Приоритет в поиске и расположение в главной карусели на 1 день</Text>
              {selectedTariff === 1 && (
                <CheckOutlined className={styles.checkIcon} />
              )}
            </Card>
